...
export function SettingsPage({ profile, user, onUpdate, onUploadAvatar, onDataDeleted }) {
  ...
  const [fieldVal3,       setFieldVal3]       = useState('') // contraseña actual (para cambio de pass)
  ...
  // Validación nueva contraseña (en modal de cambio)
  ...
  // ── Verificar contraseña actual via signIn silencioso ─────────────────────
  ...
  function openEdit(section) {
    ...
    setFieldVal2(''); setFieldVal3(''); setEditError('')
  ...
  async function handleEditSave() {
    ...
    if (editSection === 'name') {
      ...
      // fieldVal3 = contraseña actual, fieldVal = nueva, fieldVal2 = confirmar nueva
      if (!fieldVal3) { setEditError('Ingresa tu contraseña actual'); setSaving(false); return }
      if (!newPassStrong) { setEditError('La nueva contraseña no cumple todos los requisitos'); setSaving(false); return }
      if (!newPassMatch)  { setEditError('Las contraseñas nuevas no coinciden'); setSaving(false); return }
      // Verificar contraseña actual
      const valid = await verifyCurrentPassword(fieldVal3)
      if (!valid) { setEditError('La contraseña actual es incorrecta'); setSaving(false); return }
  ...
  async function handleForgotPassword() {
    ...
    else setEditError('No se pudo enviar el correo')
  }
