...
export function SettingsPage({ profile, user, onUpdate, onUploadAvatar }) {
  ...
  async function handleEditSave() {
    ...
    if (editSection === 'name') {
      ...
      await onUpdate({ name: fieldVal.trim() })
  ...
  return (
    <div style={{ paddingBottom: 120, background: 'var(--bg)', minHeight: '100vh' }}>
      ...
      <div style={{ display: 'flex', flexDirection: 'column', alignItems: 'center', marginBottom: 24 }}>
        <div style={{ position: 'relative' }}>
          ...
          <button onClick={() => fileRef.current?.click()} style={{ position: 'absolute', bottom: 0, right: 0, width: 28, height: 28, borderRadius: '50%', background: 'var(--surface)', border: '0.5px solid var(--border)', display: 'flex', alignItems: 'center', justifyContent: 'center', cursor: 'pointer' }}>
            {uploadingAvatar ? <div style={{ width: 12, height: 12, borderRadius: '50%', border: '2px solid var(--accent)', borderTopColor: 'transparent', animation: 'spin .6s linear infinite' }} /> : <Camera size={14} color="var(--text)" />}
          </button>
      ...
      <Card>
        ...
        {isGoogle && <Row label="Cuenta" value="Google" last />}
      </Card>
      ...
      <Card>
        <div style={{ padding: '13px 14px', borderBottom: '0.5px solid var(--border)' }}>
          ...
          <div style={{ display: 'flex', gap: 6 }}>
            {[['weekly','Semanal'],['biweekly','Quincenal'],['monthly','Mensual']].map(([val, label]) => (
              <button key={val} onClick={() => handleFreq(val)} style={{ flex: 1, padding: '8px 0', borderRadius: 5, border: profile.cobro_freq === val ? '1.5px solid var(--accent)' : '0.5px solid var(--border)', background: profile.cobro_freq === val ? 'var(--accent-soft)' : 'var(--bg)', color: profile.cobro_freq === val ? 'var(--accent)' : 'var(--text)', fontWeight: profile.cobro_freq === val ? 600 : 400, fontSize: 13, fontFamily: 'DM Sans, sans-serif', cursor: 'pointer' }}>
                {label}
              </button>
            ))}
          </div>
          ...
          <div style={{ padding: '13px 14px', borderBottom: '0.5px solid var(--border)' }}>
            ...
            <div style={{ display: 'flex', gap: 5, flexWrap: 'wrap' }}>
              {WEEKDAYS_SHORT.map((d, i) => (
                <button key={i} onClick={() => handleWeekday(i)} style={{ padding: '6px 10px', borderRadius: 5, border: profile.cobro_weekday === i ? '1.5px solid var(--accent)' : '0.5px solid var(--border)', background: profile.cobro_weekday === i ? 'var(--accent-soft)' : 'var(--bg)', color: profile.cobro_weekday === i ? 'var(--accent)' : 'var(--text)', fontSize: 12, fontWeight: profile.cobro_weekday === i ? 600 : 400, fontFamily: 'DM Sans, sans-serif', cursor: 'pointer' }}>
                  {d}
                </button>
              ))}
            </div>
        ...
        {(profile.cobro_freq === 'biweekly' || profile.cobro_freq === 'monthly') && (
          <div style={{ padding: '13px 14px', borderBottom: '0.5px solid var(--border)' }}>
                ...
                <div style={{ display: 'flex', gap: 6, flexWrap: 'wrap', marginBottom: 12 }}>
                  {[{label:'1 y 16',d1:1,d2:16},{label:'13 y 28',d1:13,d2:28},{label:'15 y 30',d1:15,d2:30},{label:'Personalizado',d1:null,d2:null}].map(preset => {
                    ...
                    const presetActive = isCustomPreset
                      ? !([{d1:1,d2:16},{d1:13,d2:28},{d1:15,d2:30}].some(p => p.d1===profile.cobro_day1 && p.d2===profile.cobro_day2))
                      : profile.cobro_day1 === preset.d1 && profile.cobro_day2 === preset.d2
                    return (
                      <button key={preset.label} onClick={() => { if(preset.d1!==null){onUpdate({cobro_day1:preset.d1,cobro_day2:preset.d2})} }}
                        style={{ padding: '7px 14px', borderRadius: 5, border: presetActive ? '1.5px solid var(--accent)' : '0.5px solid var(--border)', background: presetActive ? 'var(--accent-soft)' : 'var(--bg)', color: presetActive ? 'var(--accent)' : 'var(--text)', fontWeight: presetActive ? 600 : 400, fontSize: 13, fontFamily: 'DM Sans, sans-serif', cursor: 'pointer' }}>
                        {preset.label}
                      </button>
                    )
                  })}
            ...
            {profile.cobro_freq === 'monthly' && (
              <>
                <div style={{ fontSize: 13, fontWeight: 600, color: 'var(--text)', marginBottom: 8 }}>Día de cobro mensual</div>
                ...
                {profile.cobro_day1 && <div style={{ fontSize: 12, fontWeight: 400, color: 'var(--text)', marginTop: 6 }}>Cobro el día <strong>{profile.cobro_day1}</strong> de cada mes.</div>}
              </>
            )}
          </div>
        )}
      ...
      <Card>
            ...
            <div style={{ display: 'flex', alignItems: 'center', gap: 10 }}>
              ...
              <div>
                ...
                <div style={{ fontSize: 11, fontWeight: 400, color: 'var(--text)', marginTop: 1 }}>
                  {subscribed ? 'Recibes alertas de pagos en este dispositivo' : 'Recibe recordatorios de pagos'}
                </div>
              </div>
            </div>
        ...
        {subscribed && (
          <div style={{ padding: '0 14px 13px' }}>
            <div style={{ fontSize: 11, fontWeight: 500, color: 'var(--text)', marginBottom: 8 }}>Hora de notificación</div>
            <select
      ...
      <Card>
        <button onClick={handleLogout} style={{ width: '100%', display: 'flex', alignItems: 'center', gap: 10, padding: '13px 14px', background: 'none', border: 'none', cursor: 'pointer', borderRadius: 'var(--radius)' }}>
