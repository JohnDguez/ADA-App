...
// Mapa de variable CSS por categoría
const CAT_COLOR = {
...
export function GastosPage({ payments, profile }) {
  const now = new Date()
  ...
  const [catRange, setCatRange] = useState('mes') // mes | periodo | año
  ...
  function paidInMonth(month, year, cat = null) {
  ...
  function totalInMonth(month, year, cat = null) {
    return paidInMonth(month, year, cat).reduce((a, p) => a + Number(p.amount), 0)
  }

  // ── Bloque 1: datos gráfica ───────────────────────────────

  const chartMonths = getMonthsArray(monthsBack)
  const chartFiltered = selectedCat
    ? paidPayments.filter(p => p.category === selectedCat)
    : paidPayments

  function chartTotalInMonth(month, year) {
    return chartFiltered.filter(p => {
      const d = dateOf(p.due_date)
  ...
  function getCatTotal(cat) {
    ...
    if (catRange === 'periodo') {
      ...
      return paidPayments.filter(p => {
        const d = dateOf(p.due_date)
        return p.category === cat && d >= start && d <= end
      }).reduce((a, p) => a + Number(p.amount), 0)
    }
    // año
    return paidPayments.filter(p => {
      const d = dateOf(p.due_date)
      return p.category === cat && d.getFullYear() === now.getFullYear()
    }).reduce((a, p) => a + Number(p.amount), 0)
  }
  ...
  // ── Bloque 3: pagos del mes seleccionado ─────────────────
  ...
  const paidInView = paidInMonth(viewMonth, viewYear)
    .sort((a, b) => dateOf(b.due_date) - dateOf(a.due_date)) // más reciente primero
  ...
  return (
    <div style={{ paddingBottom: 120, background: 'var(--bg)', minHeight: '100vh' }}>
      ...
      {/* ── BLOQUE 1: Gráfica ── */}
      <div style={{ padding: '0 16px 4px', display: 'flex', gap: 6, overflowX: 'auto', paddingBottom: 8 }}>
      ...
      <div style={{ margin: '0 16px 20px', background: 'var(--surface)', border: '0.5px solid var(--border)', borderRadius: 'var(--radius)', padding: '16px 14px' }}>
        ...
        <div style={{ display: 'flex', alignItems: 'flex-end', gap: 6, height: 100 }}>
          {chartMonths.map((m, i) => {
            ...
            const barMuted  = selectedCat ? CAT_COLOR[selectedCat] + '66' : 'var(--accent-border)'
            return (
              <div key={i} style={{ flex: 1, display: 'flex', flexDirection: 'column', alignItems: 'center', gap: 4 }}>
                {total > 0 && (
                  <div style={{ fontSize: 9, color: isCurrent ? (selectedCat ? CAT_COLOR[selectedCat] : 'var(--accent)') : 'var(--muted)', fontWeight: 600, textAlign: 'center' }}>
                    {fmt(total)}
                  </div>
                )}
                <div style={{
                ...
                <div style={{
                  fontSize: 9,
                  color: isCurrent ? (selectedCat ? CAT_COLOR[selectedCat] : 'var(--accent)') : 'var(--muted)',
                ...
                return (
                  ...
                  >
                    ...
                    <div style={{ textAlign: 'center', minWidth: 28, flexShrink: 0 }}>
                      <div style={{ fontSize: 14, fontWeight: 700, color: 'var(--text)', lineHeight: 1 }}>{day}</div>
                      <div style={{ fontSize: 9, color: 'var(--muted)', textTransform: 'uppercase' }}>{mon}</div>
                    </div>
                    ...
                    <div style={{ flex: 1, minWidth: 0 }}>
                      ...
                      <div style={{ fontSize: 11, color: 'var(--muted)', display: 'flex', alignItems: 'center', gap: 6 }}>
                        ...
                        {p.category}
                        {p.is_recurrent && <span style={{ color: 'var(--accent)', fontSize: 10 }}>· Mensual</span>}
                      </div>
                    </div>
...
function StatCard({ label, value }) {
  return (
    <div style={{ background: 'var(--surface)', border: '0.5px solid var(--border)', borderRadius: 'var(--radius)', padding: '12px 14px' }}>
      <div style={{ fontSize: 10, fontWeight: 600, color: 'var(--muted)', textTransform: 'uppercase', letterSpacing: '0.05em', marginBottom: 3 }}>
        {label}
      </div>
