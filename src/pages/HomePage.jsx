...
import { fmt, today, nextCobroDate, isTodayCobro, getPagarEsteCobro, statusOf, daysDiff, MONTHS, WEEKDAYS } from '../lib/utils'

export function HomePage({ payments, profile, onAdd, onCardClick }) {
  const now = new Date()
  ...
  const pendingFixed = pending.filter(p => !p.is_variable)
  const pendingAmt = pendingFixed.reduce((a, p) => a + Number(p.amount), 0)
  const totalAmt = payments.filter(p => !p.is_variable).reduce((a, p) => a + Number(p.amount), 0)

  const upcoming = payments.filter(p => {
    const s = statusOf(p, profile)
    return !p.is_paid && (s === 'ok' || s === 'soon')
  ...
  return (
    <div style={{ paddingBottom: 80 }}>
      ...
      <div style={{ padding: '20px 16px 8px', display: 'flex', alignItems: 'flex-start', justifyContent: 'space-between' }}>
        <div>
          <div style={{ fontSize: 22, fontWeight: 600, color: '#1A1915' }}>
            Hola, {profile.name || 'bienvenido'}
          </div>
          <div style={{ fontSize: 13, color: '#5C5A55', marginTop: 2 }}>
            {dateStr.charAt(0).toUpperCase() + dateStr.slice(1)}
          </div>
        </div>
        <button onClick={onAdd} style={{ width: 36, height: 36, borderRadius: '50%', background: '#fff', border: '0.5px solid #E4E2DC', display: 'flex', alignItems: 'center', justifyContent: 'center', marginTop: 2 }}>
        ...
        <div style={{ margin: '8px 16px 0', background: '#FEF3DC', border: '0.5px solid #F5D9A0', borderRadius: 12, padding: '12px 14px', display: 'flex', alignItems: 'flex-start', gap: 10 }}>
          ...
          <div>
            <div style={{ fontSize: 13, fontWeight: 600, color: '#1A1915' }}>
              Recordatorio: {varReminders.map(r => r.name).join(', ')}
            </div>
            <div style={{ fontSize: 12, color: '#5C5A55', marginTop: 1 }}>
              {daysDiff(varReminders[0].due_date) === 0
                ? 'Fecha estimada hoy — tenlo contemplado'
                : `Vence en ${daysDiff(varReminders[0].due_date)} día${daysDiff(varReminders[0].due_date) !== 1 ? 's' : ''}`
              }
            </div>
          </div>
        </div>
      ...
      <div style={{ padding: '0 16px', display: 'flex', flexDirection: 'column', gap: 7 }}>
        {pagarEsteCobro.length === 0
          ...
          : pagarEsteCobro.map(p => <PayCard key={p.id} payment={p} cfg={profile} onClick={onCardClick} />)
        }
      </div>
      ...
      <div style={{ padding: '0 16px', display: 'flex', flexDirection: 'column', gap: 7 }}>
        {upcoming.length === 0
          ...
          : upcoming.map(p => <PayCard key={p.id} payment={p} cfg={profile} onClick={onCardClick} />)
        }
      </div>
    </div>
  )
...
function SectionHead({ title, action, onAction }) {
  return (
    <div style={{ padding: '16px 16px 8px', display: 'flex', alignItems: 'center', justifyContent: 'space-between' }}>
      <h2 style={{ fontSize: 10, fontWeight: 600, color: '#5C5A55', textTransform: 'uppercase', letterSpacing: '0.07em' }}>{title}</h2>
      {action && <button onClick={onAction} style={{ fontSize: 13, color: '#1E6B45', fontWeight: 500, border: 'none', background: 'none' }}>{action}</button>}
    </div>
  )
}
