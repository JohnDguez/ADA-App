...
export function VariableAmountModal({ open, payment, onConfirm, onClose }) {
  ...
  return (
    <div onClick={e => e.target === e.currentTarget && onClose()} style={{ position: 'fixed', inset: 0, background: 'rgba(26,25,21,0.45)', zIndex: 250, display: 'flex', alignItems: 'center', justifyContent: 'center', padding: '0 24px' }}>
      <div style={{ background: '#fff', borderRadius: 16, width: '100%', maxWidth: 340, padding: '24px 20px' }}>
        ...
        <div style={{ fontSize: 13, color: '#5C5A55', marginBottom: 16 }}>{payment.name} — ingresa cuánto pagaste</div>
        ...
        <button onClick={onClose} style={{ width: '100%', padding: 10, background: 'none', color: '#5C5A55', border: '0.5px solid #E4E2DC', borderRadius: 8, fontSize: 14, fontFamily: 'DM Sans, sans-serif', cursor: 'pointer' }}>
          Cancelar
        </button>
      </div>
    </div>
